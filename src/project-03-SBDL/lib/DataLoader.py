...
def read_accounts(spark, env, enable_hive, hive_db):
    ...
    if enable_hive:
        ...
        base_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'test_data', 'accounts', 'account_samples.csv'))
        return spark.read \
            .format("csv") \
            .option("header", "true") \
...
def read_parties(spark, env, enable_hive, hive_db):
    ...
    if enable_hive:
        ...
        base_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'test_data', 'parties', 'party_samples.csv'))
        return spark.read \
            .format("csv") \
            .option("header", "true") \
...
def read_address(spark, env, enable_hive, hive_db):
    ...
    if enable_hive:
        ...
        base_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'test_data', 'party_address', 'address_samples.csv'))
        return spark.read \
            .format("csv") \
            .option("header", "true") \
